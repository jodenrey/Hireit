...
export default {
  ...
  metaInfo () {
    return {
      title: 'Explore',
      meta: [
        { name: 'description', content: 'Hireit Explore: Search the latest posted projects for you to apply or peek up the best projects that have been finished.' }
      ]
    }
  },
  ...
  data: () => {
    return {
      ...
      filters: {
        expertises: [
          { name: 'Digital Art', isChecked: false, count: 0, apiName: 'DigitalArt' },
          { name: 'Photography', isChecked: false, count: 0, apiName: 'Photography' },
          { name: 'Programming', isChecked: false, count: 0, apiName: 'Programming' },
          { name: 'Video Editing', isChecked: false, count: 0, apiName: 'VideoEditing' }
        ],
        applicantTypes: [
  ...
  methods: {
    ...
    async countProject () {
      ...
      for (let project of this.projects) {
        if (project.DigitalArt) counter.expertises[0].count++
        if (project.Photography) counter.expertises[1].count++
        if (project.Programming) counter.expertises[2].count++
        if (project.VideoEditing) counter.expertises[3].count++
      }
